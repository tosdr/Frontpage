 ...
 * @author       pnoeric https://github.com/pnoeric
 * @author       JustinBack https://github.com/justinback
...
class DiscourseAPI {
    ...
    public function addTagsToTopic(array $Tags, $topic_id): stdClass {
        $params = [
            'tags' => $Tags,
        ];

        return $this->_putJsonRequest('/t/' . $topic_id, $params);
    }

    public function closeTopic($topic_id, $userName = 'system'): stdClass {
        $params = [
            'status' => "closed",
            "enabled" => "true",
        ];

        return $this->_putRequest('/t/' . $topic_id . '/status', [$params], $userName);
    }
    ...
    private function _putpostJsonRequest(
            string $requestString,
            array $paramArray,
            string $apiUser = 'system',
            $httpMethod = 'POST'
        ...
        return $this->_completeCurlCall($url, $httpMethod, $headers, json_encode($paramArray));
    }
    ...
    public function _putJsonRequest(string $reqString, array $paramArray, string $apiUser = 'system'): stdClass {
        return $this->_putpostJsonRequest($reqString, $paramArray, $apiUser, 'PUT');
    }
